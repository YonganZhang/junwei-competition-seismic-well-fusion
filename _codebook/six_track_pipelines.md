...
owner_refs:
  ...
  - test:six_track_pipeline_contract
tracked_paths_hint:
...
六个赛道共用一个入口：`python3 _code/six_track_pipeline/cli.py`。用户只需选择赛道、目标阶段和输出目录，运行时所需的前置阶段由统一计划确定，不再直接拼接各赛道的零散脚本。
...
用 `plan` 展开从起点到目标阶段的完整依赖链。下面的命令表示为六个赛道规划到 `verify`，并把计划写入指定目录：
...
用 `preflight` 检查 manifest、adapter、参数、数据前置条件和输出目录。`--param` 可重复提供，但参数必须先由相应赛道声明：
...
用 `verify` 核验已产生的阶段证据和依赖关系。它读取显式输出目录，不把“文件存在”直接等同于实验有效：
...
这四个命令承担不同职责：`list` 回答“有哪些赛道”，`plan` 回答“将按什么顺序做”，`preflight` 回答“现在能否安全开始”，`verify` 回答“已完成的阶段是否有可核验依据”。不要用 `verify` 代替实际训练，也不要用已有旧产物绕过 `preflight`。
...
正式产物和 trace 的根位置只由显式 `--output <path>` 决定。计划、预检记录、阶段结果、智能体决策、真实执行摘要和核验报告都必须能够从该目录追溯，不能依赖当前工作目录或某个脚本的隐式默认路径。

不同赛道可以在输出根目录下使用自己的子目录，但不得把临时缓存伪装成正式证据。需要复跑时，应保留原输出作为只读审计材料，并为新运行提供新的 `--output`，避免新旧 trace 相互覆盖。
