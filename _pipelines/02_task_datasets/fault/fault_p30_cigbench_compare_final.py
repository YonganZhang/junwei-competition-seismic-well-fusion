#!/usr/bin/env python3
"""Final methodologically-clean comparison for P30 on the ST10010 asset."""
...
ASSET_ROOT = TRACK_DIR / "_outputs" / "p30_3d_dev_gate_st10010"
SUBVOLUME_PATH = ASSET_ROOT / "dev_subvolume.npz"
SPLIT_MANIFEST_PATH = ASSET_ROOT / "split_manifest.json"
BASELINE_MODEL_PATH = TRACK_DIR / "_outputs" / "runs" / "audited_v2" / "baseline_model.joblib"
BASELINE_METRICS_PATH = TRACK_DIR / "_outputs" / "runs" / "audited_v2" / "baseline_metrics.json"
DEFAULT_CIG_SCALE = {"scale_t": 0.5, "scale_h": 0.85, "scale_w": 0.85}
OUTPUT_ROOT = ASSET_ROOT / "cigbench_vs_baseline_final"
COMPARE_JSON_PATH = OUTPUT_ROOT / "comparison.json"
...
def git_head() -> str:
    ...
    return (
        subprocess.check_output(["git", "-C", str(PROJECT_ROOT), "rev-parse", "HEAD"], text=True)
        .strip()
    )
...
def predict_baseline_volume(seismic: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    return probabilities, {
        ...
        "elapsed_seconds": elapsed,
        "note": "audited_v2 checkpoint evaluated slice-by-slice over the full ST10010 development cube",
    }
...
def evaluate_fold(
    ...
    *,
    ...
    truth_flat, probabilities_flat = scoreable_truth_and_probabilities(truth, fold_probabilities, score_mask)
...
def aggregate_union(results: list[dict[str, Any]], threshold: float) -> dict[str, Any]:
    truth = np.concatenate([result["truth"] for result in results]).astype(np.uint8)
    probabilities = np.concatenate([result["probabilities"] for result in results]).astype(np.float64)
...
def compare_models(
    ...
    *,
    ...
    scale = dict(DEFAULT_CIG_SCALE)
    if cig_scale is not None:
        scale.update(cig_scale)

    cig_probabilities, cig_info = predict_cigbench_volume(dev["seismic"], device=device, **scale)
    baseline_probabilities, baseline_info = predict_baseline_volume(dev["seismic"])

    cig_fit = evaluate_fold(fold_by_name["fit"], cig_probabilities)
    baseline_fit = evaluate_fold(fold_by_name["fit"], baseline_probabilities)
    cig_guard = evaluate_fold(fold_by_name["guard"], cig_probabilities, threshold=float(cig_fit["threshold"]), threshold_source="fit_reused")
    baseline_guard = evaluate_fold(fold_by_name["guard"], baseline_probabilities, threshold=float(baseline_fit["threshold"]), threshold_source="fit_reused")
    cig_validation = evaluate_fold(fold_by_name["validation"], cig_probabilities, threshold=float(cig_fit["threshold"]), threshold_source="fit_reused")
    baseline_validation = evaluate_fold(fold_by_name["validation"], baseline_probabilities, threshold=float(baseline_fit["threshold"]), threshold_source="fit_reused")

    cig_union = aggregate_union([cig_fit, cig_guard, cig_validation], float(cig_fit["threshold"]))
    baseline_union = aggregate_union([baseline_fit, baseline_guard, baseline_validation], float(baseline_fit["threshold"]))
    ...
    return {
        ...
        "p30_split_manifest": str(SPLIT_MANIFEST_PATH.relative_to(PROJECT_ROOT)),
        "p30_subvolume": str(SUBVOLUME_PATH.relative_to(PROJECT_ROOT)),
        "split": {
        ...
        "baseline_reference": {
            "audited_v2_model_path": str(BASELINE_MODEL_PATH.relative_to(PROJECT_ROOT)),
            "audited_v2_model_sha256": sha256_file(BASELINE_MODEL_PATH),
            "audited_v2_metrics_path": str(BASELINE_METRICS_PATH.relative_to(PROJECT_ROOT)),
            "audited_v2_metrics_sha256": sha256_file(BASELINE_METRICS_PATH),
            "audited_v2_old_metrics": load_json(BASELINE_METRICS_PATH)["test_metrics"],
        },
...
def render_evidence(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Final P30 CIG-Bench vs audited_v2 fault baseline comparison",
        "",
        ...
        f"- CIG-Bench scale: {json.dumps(report['cig_bench_scale'], sort_keys=True)}",
        f"- audited_v2 baseline model: `{report['baseline_reference']['audited_v2_model_path']}`",
        "",
        "## Thresholds",
        "",
        f"- CIG-Bench fit threshold: {cig['fit']['threshold']:.12g}",
        f"- Baseline fit threshold: {baseline['fit']['threshold']:.12g}",
        f"- CIG-Bench guard threshold source: `{cig['guard']['threshold_source']}`",
        f"- Baseline guard threshold source: `{baseline['guard']['threshold_source']}`",
        "",
        "## Guard metrics",
        "",
        f"- CIG-Bench: precision={cig['guard']['precision']:.6f}, recall={cig['guard']['recall']:.6f}, f1={cig['guard']['f1']:.6f}, iou={cig['guard']['iou']:.6f}",
        f"- Baseline: precision={baseline['guard']['precision']:.6f}, recall={baseline['guard']['recall']:.6f}, f1={baseline['guard']['f1']:.6f}, iou={baseline['guard']['iou']:.6f}",
        "",
        "## Validation metrics",
        "",
        f"- CIG-Bench validation: precision={cig['validation']['precision']:.6f}, recall={cig['validation']['recall']:.6f}, f1={cig['validation']['f1']:.6f}, iou={cig['validation']['iou']:.6f}",
        f"- Baseline validation: precision={baseline['validation']['precision']:.6f}, recall={baseline['validation']['recall']:.6f}, f1={baseline['validation']['f1']:.6f}, iou={baseline['validation']['iou']:.6f}",
        "",
        "## Fit/guard deltas",
        "",
        ...
        "## Conclusion",
        "",
        "The full ST10010 cube is inferred once per model, then fit/guard/validation are scored from the same probability field. The saved JSON report records the measured metrics without any separate guard-side threshold search.",
    ]
    return "\n".join(lines) + "\n"
...
def write_outputs(report: dict[str, Any], output_root: Path = OUTPUT_ROOT) -> dict[str, Any]:
    ...
    p30_manifest_path = ASSET_ROOT / "manifest.json"
    p30_manifest_sha256 = optional_sha256_file(p30_manifest_path)
    manifest = {
        "schema_version": "fault_p30_cigbench_compare_final/v1",
        "track_id": "fault",
        ...
        "inputs": [
            {
                ...
                "sha256": p30_manifest_sha256,
                "role": "P30 development gate manifest",
                "status": "present" if p30_manifest_sha256 is not None else "pending",
            },
            {"path": report["p30_subvolume"], "sha256": sha256_file(SUBVOLUME_PATH), "role": "P30 continuous 3-D development subvolume"},
            {"path": report["p30_split_manifest"], "sha256": sha256_file(SPLIT_MANIFEST_PATH), "role": "P30 development split manifest"},
        ...
        "outputs": [
            {"path": str(compare_path.relative_to(PROJECT_ROOT)), "sha256": sha256_file(compare_path), "role": "structured comparison report"},
            {"path": str(evidence_path.relative_to(PROJECT_ROOT)), "sha256": sha256_file(evidence_path), "role": "human evidence"},
        ],
...
def run(output_root: Path = OUTPUT_ROOT, device: str | None = None, cig_scale: dict[str, float] | None = None) -> dict[str, Any]:
    ...
    report = compare_models(dev, split_manifest, device=resolved_device, cig_scale=cig_scale)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--scale-t", type=float, default=DEFAULT_CIG_SCALE["scale_t"])
    parser.add_argument("--scale-h", type=float, default=DEFAULT_CIG_SCALE["scale_h"])
    parser.add_argument("--scale-w", type=float, default=DEFAULT_CIG_SCALE["scale_w"])
