...
| 科研可视化扩展 | `_pipelines/05_research_visualization_expansion/` | `render_research_figures.py` 统一渲染/验证，并将赛道⑥ P21 固定三邻域核纳入六赛道总览；`_code/visualization/geo3d_viz.py` 提供三维出版辅助；门禁 id=`research-visualization-expansion` |
...
④的 P33 runner 为 `_pipelines/02_task_datasets/lithofacies/lithofacies_p33_hybrid_agent_optimizer.py`。它进一步要求候选同时超过同预算确定性端点和当前默认 A0；本轮候选只超过前者，因而被拒绝。
