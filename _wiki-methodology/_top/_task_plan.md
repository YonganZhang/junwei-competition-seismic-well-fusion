...
> 创建: 2026-07-08
> **🧭 当前: [P45 无checkshot井震标定 issue #1七条验收标准核查+粗搜索改进方向已排除] 军伟 GitHub issue #1 驱动。I0物理baseline+I1(AI残差修正，诚实停止)+邻井先验(比AI更有效)之后，2026-08-08两轮Workflow：第一轮核查剩余5项验收标准均未通过，定位出粗相关搜索本身对噪声脆弱是核心瓶颈；第二轮针对性尝试三条打分函数改进(包络相关/带通滤波/多尺度搜索)，独立复核后**全部不采纳**，排除了"让打分函数更聪明"这一整类方向。唯一已知有效路径仍是"用邻井checkshot等外部先验约束搜索空间"，下一步应深化这个方向。（编号协调：P46 已用于甜点三层技术流，depth-local 地震适配器与③④重跑现冻结为 P47--P49，且与另一条集成线的P47--P49再次撞号，已如实记录未强行协调。）**
...
| P45 无checkshot井震标定 | ⚠️ | 7/7验收标准逐项核查+粗搜索改进3方案排除 | 军伟issue #1驱动。I0物理baseline建成；I1(AI残差修正)诚实停止(n=3井不支撑泛化)；邻井先验缩小搜索窗口比AI组件更有效(19SR MAE 1863.7→326.2ms)，但跨源鲁棒性仍不达标(316ms偏移，容差200ms)。2026-08-08两轮Workflow：①核查剩余5项验收标准(2)(3)(4)(6)+F3，独立复核确认全部诚实维持未通过/阻塞，定位出"粗相关搜索本身对噪声脆弱"是核心瓶颈；②针对性尝试三条打分函数改进(包络相关/带通滤波/多尺度粗到细搜索)，**独立复核后三个都不采纳**——包络相关全面更差(跨源偏移2792ms)、带通滤波接近中性(仅19SR部分改善)、多尺度拆东墙补西墙(19SR改善但19A/19BT2恶化)。**结论**：排除了"让打分函数更聪明"这一整类方向，唯一已知有效的仍是"用外部先验信息(邻井checkshot)约束搜索空间"，下一步应在这个方向深化，不再尝试新打分函数变体。详见`_findings/P45_well_tie_physics_baseline_no_checkshot.md`。 |
