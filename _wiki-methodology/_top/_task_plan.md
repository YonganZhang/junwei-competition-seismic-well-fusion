...
> 创建: 2026-07-08
> **🧭 当前: [P45 无checkshot井震标定 I0+I1+(c)已完成，诚实停止] 军伟 GitHub issue #1 驱动。I0物理baseline已跑通(19BT2 MAE=183.8ms通过歧义门)；I1(Ridge残差修正)诚实停止——n=3口井不足以支撑跨井泛化，暂不推进I2；(c)ST0202 vs ST10010鲁棒性测试未通过——同一套代码换地震源后t0偏移最大2716ms、2/3口井歧义判定翻转，印证了issue原话的具体担心。下一步：先修粗相关全局搜索的跨地震源稳健性，或补更多井/真实F3 LAS数据，暂不建议叠加更多AI组件。（编号协调：P46 已用于甜点三层技术流，depth-local 地震适配器与③④重跑现冻结为 P47--P49。）**
...
| P45 无checkshot井震标定 | ⚠️ | 3/3(I0+I1+c) | 军伟issue #1驱动。I0物理baseline(声波积分+bruges反射系数+Ricker子波+相关搜索+斜率约束DTW)在Volve 3口有DT+RHOB的井上测试：19BT2通过歧义门MAE=183.8ms，19A/19SR被歧义检测正确拒绝(19SR因DT+RHOB覆盖区间跟checkshot范围只有边缘重叠导致周期跳跃)。对比P23 checkshot锚定(8.7ms)仍差1-2个数量级，但比纯官方分层弱标定(633ms)有意义提升。I1(Ridge残差修正+留一井交叉验证)：首版误用井级常数特征导致19BT2灾难性外推到32332ms(已定位修复为bug)，修复后三口井留一验证全部变差(+228~+429ms)，`I1_NOT_PROMOTED_INSUFFICIENT_EVIDENCE`——n=3口井不足以支撑跨井泛化学习组件，暂缓I2。**(c)ST0202 vs ST10010鲁棒性测试(2026-08-07新增)：未通过**——同一套I0代码换地震源后t0偏移最大达2716ms，3口井里2口的歧义/接受判定本身不稳定(19BT2、19SR换源后判定翻转)，印证了issue原话"改变SEG-Y时间范围后预测明显变化"这一具体担心。**F3数据缺口**：已下载内容是图块+层位解释集，不含LAS测井曲线，跨工区冒烟测试仍不可行。详见`_findings/P45_well_tie_physics_baseline_no_checkshot.md`。下一步：先把粗相关全局搜索改成跨地震源不变量设计，而非叠加更多AI组件。 |
...
| P47 worktree 贡献审计与 R2 集成 | ✅ | 1/1 | 逐文件核对 79 个 worktree：220 个文件从未进主线（46 个 `.py`），134 处同路径分歧，涉及 52 个分支。最严重的是 P5.2 R2「有结果无代码」——主线有 26 项 R2 产物却无任何赛道实现。本轮按文件集成六赛道 R2 实现与测试（268KB），如实保留 3 处适配缺口（reservoir 8/10、lithofacies 需 tsai）。详见 `_findings/P47_worktree_contribution_audit_and_p5r2_integration.md`。 |
