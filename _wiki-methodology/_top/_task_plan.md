...
> 创建: 2026-07-08
> **🧭 当前: [P45 无checkshot井震标定 统一技术流(物理+邻井先验+自监督小模型+DTW)已跑通] 军伟 GitHub issue #1 驱动。经过I0物理baseline、I1(AI残差修正失败)、邻井先验(唯一证明有效)、两轮打分函数改进(均排除)之后，2026-08-09按用户思路搭了完整统一pipeline：自监督预训练小型编码器(4000条真实地震道、不需要标签、验证loss降8.8%、独立复核确认真实训练)+可切换打分方式+邻井先验+DTW，`run_well_tie_pipeline()`一个入口串起全部阶段，默认配置端到端跑通。自监督组件目前是混合效果(2/3井改善)，未构成净提升，"存在能跑"门槛已达到，"已经更好"门槛未达到，下一步需要更大训练量或换用项目已有MOMENT/GFM基础模型做特征提取器。（编号协调：P46 已用于甜点三层技术流，depth-local 地震适配器与③④重跑现冻结为 P47--P49，且与另一条集成线的P47--P49再次撞号，已如实记录未强行协调。）**
...
| P45 无checkshot井震标定 | ⚠️ | 统一技术流已跑通，自监督组件效果待强化 | 军伟issue #1驱动。I0物理baseline建成；I1(AI残差修正)诚实停止(n=3井不支撑泛化)；邻井先验缩小搜索窗口是目前唯一证明有效的方法。2026-08-08两轮Workflow排查7条验收标准+3种打分函数改进(均不采纳，排除"打分函数更聪明"方向)。**2026-08-09第三轮**：用户提出"物理+邻井先验+自监督小模型+DTW"统一技术流思路——用4000条ST0202真实地震道(不需要t0标签)自监督预训练一个2.8万参数小型编码器(masked reconstruction，验证loss降8.8%，独立复核确认真实训练非造假)，接入打分环节测试为混合结果(2/3井改善、19A变差，不构成净改进)，最终组装成统一入口`run_well_tie_pipeline()`(5阶段可独立开关：物理→邻井先验→打分方式可切换→DTW→歧义门)，默认配置端到端跑通复现已知最优数字。**用户要求的"小模型存在、能跑"门槛已达到；"已经带来提升"这个更高门槛尚未达到**，需要更大数据量/更长训练或换用项目已有的MOMENT/GFM基础模型做特征提取器才可能真正见效。详见`_findings/P45_well_tie_physics_baseline_no_checkshot.md`。 |
| P61 统一运行环境与源码树污染 | ✅ | 1/1 | 建立 `envs/geo-sixtrack-py311`（uv、不继承 site-packages、6.1G，清单见项目根 `requirements-sixtrack.txt`）。硬钉有据：`scikit-learn==1.7.2` 是三约束唯一交点（1.9.0 让 fault 归档 joblib 反序列化失败、tsai 1.0.1 要求 <1.8、1.7.2 与 1.8.0 数值 hash 实测一致）；torch/torchvision/monai/smp/transformers 由 facies `allowed_versions` 闸门反推；lightgbm 钉 4.6.0。6/6 硬钉精确匹配，CUDA 可用。**此前跨会话的「通过率」数字互不可比**，不应写入验收。同时修复一类源码树污染：`p10/results.py` 的 `_figure_path()` 无视调用方 `output_dir`、恒写 `OUTPUT_DIR`，导致每跑一次测试就覆盖归档图（140210→143812B）；归档图已还原。用统一环境跑 sweetspot 得 193 passed / 2 skipped / 0 failed，修复后源码树零污染。`targets/registry.py` 的同类污染（写入内容随解释器 `find_spec('resdata')` 变）仍待修。详见 `_findings/P61_unified_environment_and_source_tree_pollution.md`。 |
| P60 未集成内容清零 | ✅ | 1/1 | 复跑审计发现前几批只合了 162/220，仍有 58 个文件未进主线（含 **15 个 `.py`**，此前被我误描述为「约 100 个可再生图件」）。本批全部合入：三条赛道的 GAIA DAGT 实现与测试、`reconstruction_p6_private.py`、`visualize_property_volume.py`（43KB）、四组 P5 stage1/adapter 测试，以及 P5.2 R2 的**产物**（facies/property 的消融图、learning curve、summary/results，此前只合了 R2 代码没合结果）和 `_decisions/P5.2_r2_execution_protocol.md`。验证：15 个 py 全部编译；4 个模块可直接 import，`visualize_property_volume` 依调用方设置 sys.path（与同目录 `baseline.py` 同约定），其测试 4 passed / 1 failed（缺可选依赖 plotly）。`facies_p6_gaia_dagt.py:241` 的绝对路径经判定为 provenance 记录（与 branch/head 一同写入 resource_log.json），属历史事实，未改动。 |
| P59 同路径分歧收尾 | ✅ | 1/1 | 「两边都改」的 66 个文件逐类核实完毕，**只有 1 处是主线真缺**：`_models/property/tabiclv2_regressor.py` 的 `supported_losses`/`supported_output_activations` 声明——它与 P50 合入的 `_p5_common.py` loss/activation 机制配套，同批三个模型主线已有两个，漏了这个，已手工补齐（不覆盖主线自己的 `n_estimators`/`offload_mode` 参数校验）。其余全部无需处理：6 个 `*_agentic_optimization.yml` 主线是分支的超集（双方都做了 `ref` 改造，主线还把 `outputs_exist` 更新到 P30 新证据）；4 个 `.gitignore` 分支新增规则主线已全部包含；13 个 README/SOP 内容已同步，差异仅是标题措辞（主线的 `P5.3 / protocol R3` 比分支的 `Stage-3` 更规范）；7 个账本文件以主线为准。 |
| P58 集成源分支纠错 | ✅ | 1/1 | 对本轮合入的 74 个文件反查「其他分支是否有更新版本」，发现 3 处取错了源：`lithofacies_p5_r2.py` 及其测试应取自 `p6-gaia-lithofacies`（2026-07-27）而非 `p5-r2-lithofacies`（07-15），新版多 117 行含 `_portable_path()` 与 `_fold_from_stage3_batch()`，测试从 2 passed 增至 3 passed；`reservoir_p5_r2.py` 应取自 `p6-gaia-property`。已全部改取新版。同时确认 `historical_source_lock_mismatch` 断言在新版下仍失败——新增的 4 行只设置 `stage4_validation_status`，不改变该标志的值，印证 P47 的判断：它编码的是分支局部环境，不是代码缺陷。 |
| P57 分支侧改动三方比较与回收 | ✅ | 1/1 | 对 52 个分支的 134 处同路径分歧做三方比较（merge-base vs master vs branch）：**19 处、10 个文件是「分支改了、主线从未动过」**，123 处两边都改需人工判断，0 处是主线单方更新。已回收其中 9 个代码文件。关键收获：`_models/property/_p5_common.py` 的 `loss_name`/`output_activation` 增强（bounded 模式下 PHIF/SW 走 sigmoid、KLOGH 走 softplus 的物理有界约束）正是 P47 遗留的 `KeyError: 'loss_name'` 的原因——不是主线接口漂移，而是配套增强也留在 worktree。reservoir R2 测试因此从 8/10 升至 9/10。`reservoir/data_pipeline.py` 的「本地优先、否则回落主仓根」与 P46 修的 P28/P29 路径问题同源。回归对照：facies 126 passed；reservoir/reconstruction 的 7/6 errors 在未改动的 master 上完全相同（缺 `lasio`/`openpyxl`），与本次改动无关。`reconstruction/.gitignore` 两个候选版本均在删除 39--45 行忽略规则，已排除不取。 |
| P56 六赛道结果汇总集成 | ✅ | 1/1 | `p10-results-integration` 的 30 个文件集成入主线：`build_six_track_summary.py` 与 `six_track_model_metrics.xlsx`、`human_summary.md`、图表 manifest，并补齐 fault / reservoir / sweetspot 三条赛道的 p10 结果模块与产物。测试 5 passed / 3 skipped / 0 failed（xlsx 读写需 `openpyxl`，缺它会在 collection 阶段 ImportError，非代码故障）。`sweetspot/p10/results.py` 的 7 处本机绝对路径已改为环境变量可覆盖，默认值不变。3 个 `.gitignore` 分歧经比对为主线更新、分支落后，未取分支版本。 |
| P55 TimeLLM 基础模型改造线集成 | ✅ | 1/1 | `p6-foundation-reprogramming-pilot` 的 23 个文件集成入主线（此前主线零代码零产物）：共享 reprogramming 层、property/lithofacies 的 GPT-2 封装、两条 pilot、结果 JSON、消融图与分析报告。🔴 **它带架构匹配的随机初始化对照，是本项目目前唯一完成该对照的基础模型实验**：预训练 GPT-2 相对同架构随机 GPT-2 一致占优——property macro std RMSE `1.160538→1.089137`（相对 `13.50%`），lithofacies accuracy `0.183712→0.277778`、macro-F1 `+0.089571`、ECE `0.191935→0.084987`、NLL `2.615645→1.807097`。但 adapter 未打赢同输入简单基线，不晋级。该结论与 P8 待完成的 random-init 对照直接相关。测试 3/3 通过。详见 `_findings/P6_foundation_reprogramming_pilot.md`。 |
| P54 worktree 贡献审计与 R2 集成 | ✅ | 1/1 | 逐文件核对 79 个 worktree：220 个文件从未进主线（46 个 `.py`），134 处同路径分歧，涉及 52 个分支。最严重的是 P5.2 R2「有结果无代码」——主线有 26 项 R2 产物却无任何赛道实现。本轮按文件集成六赛道 R2 实现与测试（268KB），如实保留 3 处适配缺口（reservoir 8/10、lithofacies 需 tsai）。详见 `_findings/P54_worktree_contribution_audit_and_p5r2_integration.md`。（原编号 P47，2026-08-08 与 depth-local 系列撞号；depth-local 此前已为避让 P46 顺延过一次，故本系列让号改为 P54--P60。） |
