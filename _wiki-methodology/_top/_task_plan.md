...
> 创建: 2026-07-08
> **🧭 当前: [P45 无checkshot井震标定 I0+I1+(c)+邻井先验已完成] 军伟 GitHub issue #1 驱动。I0物理baseline已跑通；I1(AI残差修正)诚实停止——n=3口井不支撑跨井泛化；(c)ST0202 vs ST10010鲁棒性测试未通过。用户追问"AI之前传统方法怎么做"后新增：借邻井真实checkshot缩小搜索范围，2/3口井明显改善(19SR约6倍)，验证了"先验缩小搜索范围"比"叠加AI组件"更有效——这是下一步优先方向，需重新校准窄窗口下的歧义阈值。（编号协调：P46 已用于甜点三层技术流，depth-local 地震适配器与③④重跑现冻结为 P47--P49。）**
...
| P45 无checkshot井震标定 | ⚠️ | 4/4(I0+I1+c+邻井先验) | 军伟issue #1驱动。I0物理baseline在Volve 3口有DT+RHOB的井上测试：19BT2通过歧义门MAE=183.8ms，19A/19SR被歧义检测正确拒绝(19SR因覆盖区间跟checkshot范围边缘重叠导致周期跳跃)。对比P23 checkshot锚定(8.7ms)仍差1-2个数量级。I1(Ridge残差修正)：三口井留一验证全部变差，`I1_NOT_PROMOTED_INSUFFICIENT_EVIDENCE`，暂缓I2。(c)ST0202 vs ST10010鲁棒性测试：未通过，同一套代码换地震源t0偏移最大2716ms。**用户提出"AI之前人怎么做"后新增：借邻井真实checkshot做区域先验缩小搜索范围(传统方法思路)，效果明显更好**——19A MAE 265.9→88.4ms、19SR 1863.7→326.2ms(约6倍改善，解决了周期跳跃)，但19BT2反而变差(183.8→312.8ms)，且0.05歧义阈值在窄窗口场景下3口井全触发拒绝(该阈值是针对全轴盲搜校准的，需要为窄窗口场景重新校准)。**结论**：缩小搜索范围比叠加AI组件更有希望，是后续优先方向。**F3数据缺口**：不含LAS测井曲线，跨工区测试仍不可行。详见`_findings/P45_well_tie_physics_baseline_no_checkshot.md`。 |
| P53 未集成内容清零 | ✅ | 1/1 | 复跑审计发现前几批只合了 162/220，仍有 58 个文件未进主线（含 **15 个 `.py`**，此前被我误描述为「约 100 个可再生图件」）。本批全部合入：三条赛道的 GAIA DAGT 实现与测试、`reconstruction_p6_private.py`、`visualize_property_volume.py`（43KB）、四组 P5 stage1/adapter 测试，以及 P5.2 R2 的**产物**（facies/property 的消融图、learning curve、summary/results，此前只合了 R2 代码没合结果）和 `_decisions/P5.2_r2_execution_protocol.md`。验证：15 个 py 全部编译；4 个模块可直接 import，`visualize_property_volume` 依调用方设置 sys.path（与同目录 `baseline.py` 同约定），其测试 4 passed / 1 failed（缺可选依赖 plotly）。`facies_p6_gaia_dagt.py:241` 的绝对路径经判定为 provenance 记录（与 branch/head 一同写入 resource_log.json），属历史事实，未改动。 |
| P52 同路径分歧收尾 | ✅ | 1/1 | 「两边都改」的 66 个文件逐类核实完毕，**只有 1 处是主线真缺**：`_models/property/tabiclv2_regressor.py` 的 `supported_losses`/`supported_output_activations` 声明——它与 P50 合入的 `_p5_common.py` loss/activation 机制配套，同批三个模型主线已有两个，漏了这个，已手工补齐（不覆盖主线自己的 `n_estimators`/`offload_mode` 参数校验）。其余全部无需处理：6 个 `*_agentic_optimization.yml` 主线是分支的超集（双方都做了 `ref` 改造，主线还把 `outputs_exist` 更新到 P30 新证据）；4 个 `.gitignore` 分支新增规则主线已全部包含；13 个 README/SOP 内容已同步，差异仅是标题措辞（主线的 `P5.3 / protocol R3` 比分支的 `Stage-3` 更规范）；7 个账本文件以主线为准。 |
| P51 集成源分支纠错 | ✅ | 1/1 | 对本轮合入的 74 个文件反查「其他分支是否有更新版本」，发现 3 处取错了源：`lithofacies_p5_r2.py` 及其测试应取自 `p6-gaia-lithofacies`（2026-07-27）而非 `p5-r2-lithofacies`（07-15），新版多 117 行含 `_portable_path()` 与 `_fold_from_stage3_batch()`，测试从 2 passed 增至 3 passed；`reservoir_p5_r2.py` 应取自 `p6-gaia-property`。已全部改取新版。同时确认 `historical_source_lock_mismatch` 断言在新版下仍失败——新增的 4 行只设置 `stage4_validation_status`，不改变该标志的值，印证 P47 的判断：它编码的是分支局部环境，不是代码缺陷。 |
| P50 分支侧改动三方比较与回收 | ✅ | 1/1 | 对 52 个分支的 134 处同路径分歧做三方比较（merge-base vs master vs branch）：**19 处、10 个文件是「分支改了、主线从未动过」**，123 处两边都改需人工判断，0 处是主线单方更新。已回收其中 9 个代码文件。关键收获：`_models/property/_p5_common.py` 的 `loss_name`/`output_activation` 增强（bounded 模式下 PHIF/SW 走 sigmoid、KLOGH 走 softplus 的物理有界约束）正是 P47 遗留的 `KeyError: 'loss_name'` 的原因——不是主线接口漂移，而是配套增强也留在 worktree。reservoir R2 测试因此从 8/10 升至 9/10。`reservoir/data_pipeline.py` 的「本地优先、否则回落主仓根」与 P46 修的 P28/P29 路径问题同源。回归对照：facies 126 passed；reservoir/reconstruction 的 7/6 errors 在未改动的 master 上完全相同（缺 `lasio`/`openpyxl`），与本次改动无关。`reconstruction/.gitignore` 两个候选版本均在删除 39--45 行忽略规则，已排除不取。 |
| P49 六赛道结果汇总集成 | ✅ | 1/1 | `p10-results-integration` 的 30 个文件集成入主线：`build_six_track_summary.py` 与 `six_track_model_metrics.xlsx`、`human_summary.md`、图表 manifest，并补齐 fault / reservoir / sweetspot 三条赛道的 p10 结果模块与产物。测试 5 passed / 3 skipped / 0 failed（xlsx 读写需 `openpyxl`，缺它会在 collection 阶段 ImportError，非代码故障）。`sweetspot/p10/results.py` 的 7 处本机绝对路径已改为环境变量可覆盖，默认值不变。3 个 `.gitignore` 分歧经比对为主线更新、分支落后，未取分支版本。 |
| P48 TimeLLM 基础模型改造线集成 | ✅ | 1/1 | `p6-foundation-reprogramming-pilot` 的 23 个文件集成入主线（此前主线零代码零产物）：共享 reprogramming 层、property/lithofacies 的 GPT-2 封装、两条 pilot、结果 JSON、消融图与分析报告。🔴 **它带架构匹配的随机初始化对照，是本项目目前唯一完成该对照的基础模型实验**：预训练 GPT-2 相对同架构随机 GPT-2 一致占优——property macro std RMSE `1.160538→1.089137`（相对 `13.50%`），lithofacies accuracy `0.183712→0.277778`、macro-F1 `+0.089571`、ECE `0.191935→0.084987`、NLL `2.615645→1.807097`。但 adapter 未打赢同输入简单基线，不晋级。该结论与 P8 待完成的 random-init 对照直接相关。测试 3/3 通过。详见 `_findings/P6_foundation_reprogramming_pilot.md`。 |
| P47 worktree 贡献审计与 R2 集成 | ✅ | 1/1 | 逐文件核对 79 个 worktree：220 个文件从未进主线（46 个 `.py`），134 处同路径分歧，涉及 52 个分支。最严重的是 P5.2 R2「有结果无代码」——主线有 26 项 R2 产物却无任何赛道实现。本轮按文件集成六赛道 R2 实现与测试（268KB），如实保留 3 处适配缺口（reservoir 8/10、lithofacies 需 tsai）。详见 `_findings/P47_worktree_contribution_audit_and_p5r2_integration.md`。⚠️2026-08-08合并时发现：本行P47（worktree贡献审计）跟下面第128行"P47深度局部接口门"（depth-local系列，此前为避让P46已顺延到P47起）撞号，P48/P49也各自撞了两次。合并时未重新协调，只如实记录，留给下一位处理者判断谁该再改号。 |
