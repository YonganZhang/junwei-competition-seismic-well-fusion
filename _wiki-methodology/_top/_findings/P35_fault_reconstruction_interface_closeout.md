...
P29 v2 在五个 held folds 上完成真实 DeepSeek 决策，10 次 provider 调用全部成功。修复后的 A0 RMSE 为 `0.027734374378`，与 P21 一致；策略仅在 2/5 折改善，平均 signed RMSE delta 为 `+0.000181521`，因此确定性门禁保留 P21。P30 v2 是固定五折、每折 512 个训练标签和 2,048 个验证行预算下的稀疏 Eclipse 网格代理实验；各向异性普通克里金 RMSE 为 `0.030569516403`，回归克里金代理为 `0.030093884156`，均未超过 P21。它不是真实测井驱动的克里金或协同克里金实验。

本轮已经排除 foundation 特征混入 P30 候选、输入列错位以及 target/KJI 错位，并确认 P29 v2 正确传递训练侧与查询侧模态、执行真实动作；这只能约束当前固定 split、动作空间和数据预算，不能外推为传统地质统计学被否定，也不能声称所有潜在接口问题均已排除。P30 covariance-form ordinary kriging 方差中的 Lagrange multiplier 符号 bug 已由 `+mu` 修为数学正确的 `-mu`；该修复不影响 kriging weights、均值预测、RMSE 或晋级决定，P21 仍为默认模型。
