...
「AI 增益」一列区分**已通过晋级门**和**试过但没赢**。两者都是有效结果，但只有前者可以
对外声称改进。不要把「接线成功」「候选真实执行」当成「指标提升」。

| 赛道 | 当前默认 | AI 增益是否成立 | 展开 |
|---|---|---|---|
| ② 地震相 | P32 混合优化（F3 联合配置 + Penobscot A0）| ✅ 等均 mIoU `0.301022749 → 0.325721341` | P32 |
| ③ 储层物性 | P32 候选生成 + 确定性调度 | ✅ 复合 RMSE 改善 `4.2696%`，3/3 seed 全胜 | P32、P41 |
| ⑤ 甜点 | T3 由 Chronos-2 晋级；T1/T2/T4 为 P5 小模型 | ✅ T3 MAE `267.118 → 186.572`（`-30.15%`），`PROMOTE` | P43、P44 |
| ④ 岩相 | XGBoost `depth=3/eta=0.1/rounds=60` | ⚠️ 默认调参有效，但双基础融合 `0.161312` < B0 `0.213349` 不晋级 | P33 |
| ⑥ 三维重建 | P21 固定三核集成 | ❌ PEFT/对比残差/checkshot/井震融合均未超过 P21 | P19–P24、P37–P39 |
| ① 断层 | 局部 baseline + A2D 决策治理 | ❌ CIG-Bench 对齐后信号可测但精度不足 | P25.1 |

跨赛道：混合智能体（LLM 提候选 + 确定性执行 + 独立晋级门）在②③有效；**直接让 LLM 做数值裁决
在六赛道均无稳定优势**。冻结双基础模型融合在③④⑥三条赛道全部未过门。
