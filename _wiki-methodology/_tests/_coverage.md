...
- 赛道⑥已从 `CONNECTED_UNVERIFIED` 推进到一条合法的 `DEVELOPMENT_SIGNAL`：P17 的 GFM 非平稳邻域路线相对 PyKrige RMSE -0.4563%，3/5 折改善。整折 bootstrap 95% 区间仍跨 0，且用户要求将消融后置，因此默认仍关闭、不归因预训练贡献。其他赛道的同 split 控制门仍需分别验证。
